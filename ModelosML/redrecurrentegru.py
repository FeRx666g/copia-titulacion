...
# =============================================
# PASO 2: Definir y entrenar modelo GRU
# =============================================
...
for k, v in resultados_gru.items():
    print(f"{k}: {v}")
...
plt.title('Pérdida MSE - Entrenamiento vs Validación (GRU)')
plt.xlabel('Épocas')
plt.ylabel('Pérdida')
plt.legend()
plt.grid(True)
...
plt.title('Potencia real vs predicha (GRU)')
...
plt.title('Potencia predicha por la red GRU')
plt.xlabel('Índice de muestra')
plt.ylabel('Potencia (W)')
plt.grid(True)
plt.tight_layout()
