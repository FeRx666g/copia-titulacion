...
export const PrediccionEstatica = () => {
  ...
  const getHoraLabel = (idx) => {
    ...
    const hora =
      fila.hora !== undefined && fila.hora !== "" ? fila.hora : res.hora;
    const minuto =
      fila.minuto !== undefined && fila.minuto !== "" ? fila.minuto : res.minuto;

    const hh =
      hora !== undefined && hora !== null
        ? hora.toString().padStart(2, "0")
        : "--";
    const mm =
      minuto !== undefined && minuto !== null
  ...
  // 5. Validación: Si no hay usuario, bloqueamos la vista y no renderizamos nada más
  if (!user) {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black p-4 m-2 flex flex-col items-center">
      ...
      <div className="tour-chart-static relative mt-8 w-full max-w-7xl shadow-xl rounded-lg bg-white p-4">
        <div className="absolute top-2 right-2 z-50 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-sm border border-gray-300 dark:border-zinc-700 rounded-lg px-4 py-2 shadow-md text-xs leading-tight max-w-[320px] pointer-events-none">
          ...
          <p className="text-gray-700 dark:text-gray-200">
            <span className="font-semibold">Modelo:</span> Random Forest
          </p>
          <p className="text-gray-700 dark:text-gray-200">
            <span className="font-semibold">Error Absoluto Medio (MAE):</span>{" "}
            6.0685 mW
          </p>
          <p className="text-gray-700 dark:text-gray-200">
            <span className="font-semibold">Error Cuadrático Medio (MSE):</span>{" "}
            659.1507 mW²
          </p>
          <p className="text-gray-700 dark:text-gray-200">
